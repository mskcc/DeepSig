...
train, test = train_test_split(ex, test_size=0.2, random_state=1)
...
test_X = tf.convert_to_tensor(np.array(test),dtype=tf.float64)
...
pred = pd.concat([pd.DataFrame(test_Y), pred], axis=1)
pred.to_csv('output/predict_'+S+'.csv', index=False)
