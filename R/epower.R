...
# Read reference signatures
  if(!reference %in% c('v2','SA')) stop('Reference other than v2 or SA not implemented')
    ...
    ref <- read.table(system.file('extdata', 'cosmic_snv_signatures_v2.txt', package ='DeepSig'),
                      header = TRUE, sep = '\t')
  ...
  if(is.null(Mcoeff)){
    if(reference=='v2')
      mf <- read.table(system.file('extdata', 'Mcoeff_v2.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
    else
      mf <- read.table(system.file('extdata', 'Mcoeff.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
  } else mf <- Mcoeff
  ...
  # Read parameter sets
    if(reference=='v2'){
      if(is.null(afit)) afit <- read.table(system.file('extdata', 'Acoeff_v2.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
      if(is.null(bfit)) bfit <- read.table(system.file('extdata', 'Bcoeff_v2.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
    } else{
      if(is.null(afit)) afit <- read.table(system.file('extdata', 'Acoeff.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
      if(is.null(bfit)) bfit <- read.table(system.file('extdata', 'Bcoeff.txt', package = 'DeepSig'), header = TRUE, sep = '\t')
#    fmin <- read.table(system.file('extdata', 'Fmin.txt', package = 'DeepSig'), 
#                       header = TRUE, sep = '\t')
    }
    nsig <- length(sbs)
...
#     }
    ...
    for(iM in seq_along(Msamp)){
      ...
      ts <- DeepSig(data = xcat, signat=ref)
